...
layout(location = 9) uniform sampler2D colorMap;
...
vec3 lightColor = vec3(0.95, 0.85, 0.2);
...
//Blinn-Phong shading values
    ...
    outColor = vec4(texture(colorMap, vec2(1.0 -fragTexCoord.x, fragTexCoord.y)).rgb*visibility*(ambientColor+lightColor*specular*vec3(max(dot(fragNormal,lightDir),0.0))),1.0);
